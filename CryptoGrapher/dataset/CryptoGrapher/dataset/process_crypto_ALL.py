"""Convert all daily Binance_USDT CSV files into StockMixer-format pickle files."""
...
def main():
    print("=" * 80)
    print("Binance_USDT to StockMixer Format Conversion")
    print("=" * 80)
    ...
    print(f"\nData Statistics:")
    print(f"  Original coins: {len(all_symbols)}")
    print(f"  Excluded stablecoins: {len(exclude)} ({', '.join([c.replace('USDT','') for c in exclude])})")
    print(f"  Final coins: {len(coin_symbols)}")
    ...
    for i, symbol in enumerate(coin_symbols, 1):
        ...
        try:
            ...
            if i % 10 == 0:
                print(f"  [{i}/{len(coin_symbols)}] Loaded {i} coins")
        except Exception as e:
            print(f"  [FAIL] {symbol}: {str(e)[:50]}")
            failed.append(symbol)

    print(f"\n  Success: {len(crypto_data)}/{len(coin_symbols)}")
    if failed:
        print(f"  Failed ({len(failed)}): {', '.join(failed[:5])}")

    print(f"\n{'='*80}")
    print("Step 2/4: Time Alignment")
    print(f"{'='*80}")
    ...
    print(f"  Coverage: {len(common_dates)/1000*100:.1f}%")
    ...
    print(f"\n{'='*80}")
    print("Step 3/4: Format Conversion")
    print(f"{'='*80}")
